...
real(8),parameter ::                OmegaM = 0.3 
real(8),parameter ::                OmegaA = 1.- OmegaM
real(8),parameter ::                Hub    = 100.!67.0
real(8),dimension(3),parameter::    bulk_flow = (/ 0., 0. ,0. /)
! Please setting the directories of the input and output.
! These are the only thing you need to modify in this code.
! Do not modify the rest of the code. 
PS_Multi         = 'ALL'  !  'YES' ! 'NO'   !             do you want to calculate l=1,2,3,4 ? 
Nmock            =  10           ! how many mocks you will use? 
Nrand            =  1
file_dir         =  '/Users/fei/WSP/Scie/FitExamp/Data/ProdFt/'
...
print*,'Nrand=',nrand
print*,''
print*, 'Please make sure that in the catalogue,'
print*, 'observer is at Coordinate origin=[0,0,0].'  
print*,''
...
!
!------------------------------------------------------------------------------
Input_dir        = trim(adjustl(file_dir))//trim(adjustl('Data'))
Input_dir_pv     = trim(adjustl(file_dir))//trim(adjustl('Datav'))
Input_rand_dir   = trim(adjustl(file_dir))//trim(adjustl('Rand'))
Input_mock_dir   = trim(adjustl(file_dir))//trim(adjustl('Mock_'))
Input_mock_dir_pv= trim(adjustl(file_dir))//trim(adjustl('Mockv_'))
Input_rand_for_mock_dir = trim(adjustl(file_dir))//trim(adjustl('MockRand_'))
...
Do icod= 1,6
    ...
    IF(do_mock==0)then
        ...
        if((do_mom == 0) .or. (do_mom==2))then          
            ...
            if(do_mom==0)then 
                allocate(NAN(ndata)) 
                call cosmic_PS(do_mom , PS_Multi,output,   &
        ...
        if((do_mom == 1) .or. (do_mom==2))then   
            ...
            vpec  =  log(10.)*Vmod/(1.0+Vmod/cl) * logd &
                  -  ( bulk_flow(1)*xpv + bulk_flow(2)*ypv + bulk_flow(3)*zpv )/sqrt(  xpv**2+ypv**2+zpv**2 )        
