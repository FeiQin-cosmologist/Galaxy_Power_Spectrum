...
! Do not modify the rest of the code. 
character(1000)      ::    file_dir  = '/Users/fei/WSP/Scie/FitExamp/Data/ProdFt/'
...
if(do_mom==1)then
    Input_rand= trim(adjustl(file_dir))//trim(adjustl('Randv'))  
endif
if(do_mom==0)then
    Input_rand= trim(adjustl(file_dir))//trim(adjustl('Rand'))  
endif
...
DO i=1,ndataR
if(do_mom==1)then
    read(134,*)longitude_rand(i),latitude_rand(i),Vcmb_rand(i),var,&
...
print*,'Randoms'
nbr=nbar_rand
call rand_PS(do_mom,ndataR,nbr,Xr,Yr,Zr,wr,output_dir)

deallocate(nbr);deallocate(wr);
deallocate(longitude_rand);deallocate(latitude_rand);
deallocate(Vcmb_rand);deallocate(nbar_rand);
deallocate(wden_rand);deallocate(wmom_rand);
deallocate(disr);
