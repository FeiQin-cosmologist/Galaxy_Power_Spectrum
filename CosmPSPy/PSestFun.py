...
def Pkest_Fun(kmin,kmax,nk,nx,ny,nz,xmin,xmax,ymin,ymax,zmin,zmax,  
    ...
    if((PS_type == 'den') or (PS_type == 'crs')):
        FUN,edges = np.histogramdd(np.vstack([x,y,z]).transpose()   , bins=(nx,ny,nz), range=((-lx/2.,lx/2.),(-ly/2.,ly/2.),(-lz/2.,lz/2.)), weights=w        )
        alpha     = np.sum(w)/np.sum(wr)
        tmp,edges = np.histogramdd(np.vstack([xr,yr,zr]).transpose(), bins=(nx,ny,nz), range=((-lx/2.,lx/2.),(-ly/2.,ly/2.),(-lz/2.,lz/2.)), weights=alpha*wr )
        FUN       = FUN-tmp 
    if((PS_type == 'mom') or (PS_type == 'crs')):
        if(PS_type == 'mom'):
            FUN,edges    = np.histogramdd(np.vstack([xpv,ypv,zpv]).transpose(), bins=(nx,ny,nz), range=((-lx/2.,lx/2.),(-ly/2.,ly/2.),(-lz/2.,lz/2.)), weights=wmom*vp )
        if(PS_type == 'crs'):
            FUNmom,edges = np.histogramdd(np.vstack([xpv,ypv,zpv]).transpose(), bins=(nx,ny,nz), range=((-lx/2.,lx/2.),(-ly/2.,ly/2.),(-lz/2.,lz/2.)), weights=wmom*vp )
    edges=0.;tmp=0.
    ...
    if(PS_type =='den'):
        Pnoise = np.sum(w*w)             ; PnoiseR = alpha*alpha*np.sum(wr*wr) ;  Norm = np.sum(nb*w*w)  
    if (PS_type=='mom'):
        Pvnoise= np.sum(wmom*wmom*vp*vp) ; Norm = np.sum(nbmom*wmom*wmom)  
    if (PS_type=='crs'):
        PnoiseC= np.sum(wrho*wmom*vp)    ; Norm = np.sqrt( np.sum(nb*w*w))*np.sqrt(np.sum(nbmom*wmom*wmom)) 
    ...
    if((PS_multi!='no')):
      for i in range(nk):
        if(i==nk-1):
            outfile.write("  %7d     %17.10lf     %7d     %7d     %7d     %7d     %7d     %7d     %7d \n"%(i+1, kmin+(i+0.5)*dk,0,0,0,0,0,0,0))
        else:
            outfile.write("  %7d     %17.10lf     %17.10lf     %17.10lf     %17.10lf     %17.10lf     %17.10lf    %7d     %30.20lf \n"%(i+1, kmin+(i+0.5)*dk,np.real(p0[i]),np.real(p1[i]),np.real(p2[i]),np.real(p3[i]),np.real(p4[i]),Nbink[i],Norm))
    if(PS_multi=='no'):
      for i in range(nk):
        if(i==nk-1):
          outfile.write("  %7d     %17.10lf     %7d     %7d     %7d     %7d     %7d     %7d     %7d \n"%(i+1, kmin+(i+0.5)*dk,0,0,0,0,0,0,0))
        else:
          outfile.write("  %7d     %17.10lf     %17.10lf     %7d     %7d     %7d     %7d     %7d     %30.20lf \n"%(i+1, kmin+(i+0.5)*dk,np.real(p0[i]),0,0,0,0,Nbink[i],Norm))
    outfile.close()   
    if(PS_type == 'den'):
        weifkp=[w,wr]
    if(PS_type == 'mom'):
...
def PkestR_Fun(kmin,kmax,nk,nx,ny,nz,xmin,xmax,ymin,ymax,zmin,zmax,ndata,longir,latir,rsfr,epv,nbr,FKP,OmegaM,OmegaA,Hub,sigv,PS_type,file_dir='PSestDir'):
    if((nx % 2)!=0):
        print('\n Error: nx should set to be a even number.\n')
        sys.exit()
    if((ny % 2)!=0):
    ...
    dx=(xmax-xmin)/nx; dy=(ymax-ymin)/ny; dz=(zmax-zmin)/nz
    lx=np.abs(xmax-xmin) ; ly=np.abs(ymax-ymin) ; lz=np.abs(zmax-zmin) 
    FUN,edges = np.histogramdd(np.vstack([xr,yr,zr]).transpose(),bins=(nx,ny,nz),range=((-lx/2.,lx/2.),(-ly/2.,ly/2.),(-lz/2.,lz/2.)),weights= wr)
    # 2: shotnoise:
    PnoiseR= np.sum(wr*wr)
