...
def CHI2(params,    ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type ):
    params = ParmPSmod_Fun(params)
    Pk_modc, kmodc, Pk_mod , kmod =PkmodMulti_Fun(params,OnlyL0,Sig8_fid, k_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type )
    if(OnlyL0):
...
def tPDF(params,    Cp,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type ):
    ...
    return -Pb
def Box_CoxFUN(params ,BC_nu,BC_d,Cp,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs,Pk_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type ):
    params = ParmPSmod_Fun(params)
    Pk_modc, kmodc,  Pk_mod , kmod  =PkmodMulti_Fun(params,OnlyL0,Sig8_fid, k_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type )
    if(OnlyL0):    
...
def ZeroPoint( S, Cinv,   x ):
    ...
    return ln_like
def zpPDF(params    ,x,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type):
    params = ParmPSmod_Fun(params)
    Pk_modc, kmodc, Pk_mod , kmod =PkmodMulti_Fun(params,OnlyL0,Sig8_fid, k_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type )
    Pk_obs,Pk_modc=PSmulti_pickup(ind_fit,ps_type,Pk_obs,Pk_modc)
...
def lnprior(params):
    fsigma8,  b1sigma8,b2sigma8,b3nlsigma8,  b1vsigma8, sigmav1_squre,sigmav2_squre,sigmavv_squre = params   
...
def lnpost_chi2(params    ,   Cp,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type  ):
    ...
    return prior + like
def lnpost_zp(params    , x,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type ):
    prior = lnprior(params )
    if not np.isfinite(prior):
        return -np.inf
    like = -1. *  zpPDF(params    ,x,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type)
    return prior + like
# 5.2: the post of MCMC for box-cox trans:
def lnpost_bc(params ,BC_nu,BC_d,Cp,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs,Pk_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type  ):
    prior = lnprior(params )
    if not np.isfinite(prior):
        return -np.inf
    like = Box_CoxFUN(params,BC_nu,BC_d,Cp,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs,Pk_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type   )
    return prior - like

# 5.3: fit func:---------------------------------------------------------------
def Param_Estimate(fit_minK,fit_maxK,Optp ,MCp,PS_type,fit_tech,strategy,OnlyL0,Sig8_fid,k_obs,Pk_obs,k_mocks,Pk_mocks,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,  Growthz, WFCon_type):
...
# 6.2 chi2 minimization:---------------------------------
    if(fit_tech=='Chi2'):
        ...
        if(strategy=='Optimize'):
            ...
            outpf    = sp.optimize.minimize(tPDF,[ Optp[0],Optp[1],Optp[2],Optp[3],Optp[4],Optp[5],Optp[6],Optp[7]  ],args=(Cp,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,PS_type, Growthz, WFCon_type ),method='Nelder-Mead',tol=10.**(-200))
            fsigma8  = outpf.x[0]
            b1sigma8 = outpf.x[1];b2sigma8= outpf.x[2];b3nlsigma8= outpf.x[3]; 
            b1vsigma8= outpf.x[4]; 
            sigmav1_squre= outpf.x[5];sigmav2_squre= outpf.x[6];
            sigmavv_squre= outpf.x[7];
            chisq    = CHI2(outpf.x,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,PS_type, Growthz, WFCon_type )
            return    fsigma8,  b1sigma8,b2sigma8,b3nlsigma8,  b1vsigma8, sigmav1_squre,sigmav2_squre,sigmavv_squre,    chisq 
        if(strategy=='MCMC'):
            print( 'strategy = MCMC\n')
            ndim, nwalkers = 8, 30
            ...
            sampler = emcee.EnsembleSampler(nwalkers, ndim, lnpost_tPDF, args=[Cp,Nmock, ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,PS_type, Growthz, WFCon_type])
    ...
    if(fit_tech=='ZP'):
        ...
        if(strategy=='Optimize'):
            print( 'strategy = Optimize\n')          
            outpf    = sp.optimize.minimize(zpPDF,[ Optp[0],Optp[1],Optp[2],Optp[3],Optp[4],Optp[5],Optp[6],Optp[7] ],args=(x,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,PS_type, Growthz, WFCon_type ),method='Nelder-Mead',tol=10.**(-200))
            fsigma8  = outpf.x[0]
            b1sigma8 = outpf.x[1];b2sigma8= outpf.x[2];b3nlsigma8= outpf.x[3]; 
            b1vsigma8= outpf.x[4]; 
            sigmav1_squre= outpf.x[5];sigmav2_squre= outpf.x[6];
        ...
        if(strategy=='MCMC'):
            ...
            begin = [[(0.05*(np.random.rand()-0.5)+Optp[0]),
                      ...
                      (0.1*(np.random.rand()-0.5)+Optp[6]),
                      (0.1*(np.random.rand()-0.5)+Optp[7])] for i in range(nwalkers)]  
            sampler = emcee.EnsembleSampler(nwalkers, ndim, lnpost_zp, args=[x,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs, Pk_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,PS_type, Growthz, WFCon_type])
            print('   ... mcmc initial sampling')
            pos, prob, state = sampler.run_mcmc(begin, MCp[0])
            sampler.reset()
            print('   ... mcmc final sampling')
    ...
    if(fit_tech=='BC'):
        ...
        if(strategy=='Optimize'):
            print( 'strategy = Optimize\n')        
            outpf    = sp.optimize.minimize(Box_CoxFUN,[ Optp[0],Optp[1],Optp[2],Optp[3],Optp[4],Optp[5],Optp[6],Optp[7] ],args=( BC_nu,BC_d,Cp,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs,Pk_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,PS_type, Growthz, WFCon_type ),method='Nelder-Mead',tol=10.**(-200))
            fsigma8  = outpf.x[0]
            b1sigma8 = outpf.x[1];b2sigma8= outpf.x[2];b3nlsigma8= outpf.x[3]; 
            b1vsigma8= outpf.x[4]; 
            sigmav1_squre= outpf.x[5];sigmav2_squre= outpf.x[6];
            sigmavv_squre= outpf.x[7];
            chisq    = CHI2(outpf.x,ind_fit,Cov_inv0,OnlyL0,Sig8_fid, k_obs, Pk_obs0,kmod_conv, WF_Kobs_Kmod,WF_rand,PT,PS_type, Growthz, WFCon_type )
            return    fsigma8,  b1sigma8,b2sigma8,b3nlsigma8,  b1vsigma8, sigmav1_squre,sigmav2_squre,sigmavv_squre,    chisq         
        if(strategy=='MCMC'):
            print( 'strategy = MCMC\n')
            ndim, nwalkers = 8, 30
            begin = [[(0.05*(np.random.rand()-0.5)+Optp[0]),
                      ...
                      (0.1*(np.random.rand()-0.5)+Optp[6]),
                      (0.1*(np.random.rand()-0.5)+Optp[7])] for i in range(nwalkers)]     
            sampler = emcee.EnsembleSampler(nwalkers, ndim, lnpost_bc, args=[BC_nu,BC_d,Cp,Nmock,ind_fit,Cov_inv,OnlyL0,Sig8_fid, k_obs,Pk_obs, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,PS_type, Growthz, WFCon_type])
            print('   ... mcmc initial sampling')
            pos, prob, state = sampler.run_mcmc(begin, MCp[0])
            sampler.reset()
            print('   ... mcmc final sampling')
...
######                                                                   ######
###############################################################################                 
def MCplot(mean,chain,savr):
    c = ChainConsumer().add_chain( chain , parameters=[ '$f\sigma_8$',    
            '$b^{\delta}_1\sigma_8$', '$b^{\delta}_2\sigma_8$','$b^{\delta}_{3nl}\sigma_8$',
            '$b^p_1\sigma_8$', '$\sigma^2_{1v}$' ,'$\sigma^2_{2v}$' ,'$\sigma^2_{pv}$' ]) 
    c.configure(flip=False,  colors="#00bfa5",tick_font_size=14,label_font_size=19, cloud=True, sigma2d=True, sigmas=[1,1.5,2,2.5])
    fig = c.plotter.plot(figsize=(12,12),filename=savr,truth=mean)
    plt.show()
    c = ChainConsumer().add_chain( chain[:,:4] , parameters=[ '$f\sigma_8$',    
            '$b^{\delta}_1\sigma_8$', '$b^{\delta}_2\sigma_8$','$b_{3nl}\sigma_8$' ]) 
    c.configure(flip=False,  colors="#00bfa5",tick_font_size=14,label_font_size=19, cloud=True, sigma2d=True, sigmas=[1,1.5,2,2.5])
    fig = c.plotter.plot(figsize=(12,12),filename=savr,truth=mean)
    plt.show()
    c = ChainConsumer().add_chain( chain[:,4:] , parameters=[  '$b^p_1\sigma_8$', 
             '$\sigma^2_{1v}$' ,'$\sigma^2_{2v}$' ,'$\sigma^2_{pv}$' ]) 
    c.configure(flip=False,  colors="#00bfa5",tick_font_size=14,label_font_size=19, cloud=True, sigma2d=True, sigmas=[1,1.5,2,2.5])
    fig = c.plotter.plot(figsize=(12,12),filename=savr,truth=mean)
    plt.show()
 
def Plot_l0(params , Nk,fit_minK,fit_maxK,Sig8_fid, k_data,Pk_data,ekPk,ePk, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type,iplt ): 
    params = ParmPSmod_Fun(params)
    if('den' in ps_type ) and('mom' not in ps_type):
        Pkmodc, kmodc, Pkmod , kmod =PkmodMulti_Fun(params,True,Sig8_fid, k_data, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,ps_type, Growthz, WFCon_type )
...
def Plot_denMulti(param_test ,Nk,fit_minK,fit_maxK,k_data,Pk_data,ePk,ekPk,Sig8_fid,PT,PS_type, Growthz,kmod_conv,WF_Kobs_Kmod,WF_rand, WFCon_type ,iplt ):    
    param_test = ParmPSmod_Fun(param_test)
    ...
    Pkmodc , kmodc , Pkmod , kmod  =PkmodMulti_Fun(param_test,False,Sig8_fid, k_data, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,'den-024', Growthz, WFCon_type )
    Nkm=len(kmod)  //3
    k_data = k_data[:Nk]
    kmod   = kmod[:Nkm]
    kmodc  = kmodc[:Nk]
...
def Plot_momMulti(param_test ,Nk,fit_minK,fit_maxK,k_data,Pk_data,ePk,ekPk,Sig8_fid,PT,PS_type, Growthz,kmod_conv,WF_Kobs_Kmod,WF_rand, WFCon_type ,iplt ):    
    param_test = ParmPSmod_Fun(param_test)
    Ps_type = PS_type.split(' ')
    ps_type=[]
    mt=[]
    ...
    for i in range(len(Ps_type) ):
        ...
        ps_type.append(ps_type0[0]) 
        Nmt.append( len( ps_type0[1]))   
    Pkmodc , kmodc , Pkmod , kmod  =PkmodMulti_Fun(param_test,False,Sig8_fid, k_data, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,'mom-024', Growthz, WFCon_type )
    Nkm=len(kmod)  //3
    k_data = k_data[:Nk]
    kmod   = kmod[:Nkm]
    kmodc  = kmodc[:Nk]
...
def Plot_crsMulti(param_test ,Nk,fit_minK,fit_maxK,k_data,Pk_data,ePk,ekPk,Sig8_fid,PT,PS_type, Growthz,kmod_conv,WF_Kobs_Kmod,WF_rand, WFCon_type ,iplt ):    
     param_test = ParmPSmod_Fun(param_test)
     Ps_type = PS_type.split(' ')
     ps_type=[]
     mt=[]
     ...
     for i in range(len(Ps_type) ):
         ...
         ps_type.append(ps_type0[0]) 
         Nmt.append( len( ps_type0[1]))   
     Pkmodc , kmodc , Pkmod , kmod  =PkmodMulti_Fun(param_test,False,Sig8_fid, k_data, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,'crs-13', Growthz, WFCon_type )
     Nkm=len(kmod)  //2 
     k_data = k_data[:Nk]
     kmod   = kmod[:Nkm]
     kmodc  = kmodc[:Nk]
