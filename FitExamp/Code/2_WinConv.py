...
WFCon_type  = 'CosmPS'  # 'Ross'     #    'Pypower' #
PS_type     =   'den'    # 'mom'    #    'crs'      #       
...
FKP,  FKPv  = 1600 , 5.0e9 
NczBin      = 50
frac_nyq    = 0.9                    # this is for Pypower method only.
if(PS_measure  == 'survey' ):
    randg_Ratio, randv_Ratio  =  116.59168901525683,  1688.2354530106923
    WeitRg_Ratio,WeitRv_Ratio =  112.75647879064488,  1657.2744260916527     # this is for Pypower method only.
    Pkden_norm ,Pkmom_norm ,Pkcrs_norm = 9.37848, 3.25087*10**(-13), 1.746089*10**(-6) # this is for Pypower method only.  
if(PS_measure  == 'mocks' ):
    randg_Ratio, randv_Ratio  =  110.16111933176775, 1688.2354530106927
    WeitRg_Ratio,WeitRv_Ratio =  114.50254965704735, 1655.5829495778107     # this is for Pypower method only.
    Pkden_norm ,Pkmom_norm ,Pkcrs_norm = 9.40913, 3.25740*10**(-13), 1.750615*10**(-6) # this is for Pypower method only. they are average of the 600 mocks. 
...
# 2.2 Black method:--------------
if(WFCon_type == 'CosmPS')  :
    if(PS_type=='den'):
        conv,kd,kc   =  ConvMat_CosmPS(kmin,kmax,nkbin,kmaxc,nkbinc,nx,ny,nz,lx,ly,lz,weigridgal,wingridgal ,weigridgal,wingridgal,PS_type) 
    if(PS_type=='mom'):
        conv,kd,kc   =  ConvMat_CosmPS(kmin,kmax,nkbin,kmaxc,nkbinc,nx,ny,nz,lx,ly,lz,weigridpv ,wingridpv  ,weigridpv ,wingridpv ,PS_type) 
    if(PS_type=='crs'):
        conv,kd,kc   =  ConvMat_CosmPS(kmin,kmax,nkbin,kmaxc,nkbinc,nx,ny,nz,lx,ly,lz,weigridgal,wingridgal ,weigridpv ,wingridpv ,PS_type) 
...
if(WFCon_type == 'Pypower'):
    ...
    if(PS_type=='den'):
        conv,kd,kc   = ConvMat_Pypower( randg_x ,randg_y,randg_z,randg_weit,Ngrid,Pkden_norm,WeitRg_Ratio,kmin,kmax,nkbin,kminc,kmaxc,nkbinc,[0,2,4],boxsize,boxsizes,frac_nyq, PS_type,WFCon_type)
    if(PS_type=='mom'):
        conv,kd,kc   = ConvMat_Pypower( randv_x ,randv_y,randv_z,randv_weit,Ngrid,Pkmom_norm,WeitRv_Ratio,kmin,kmax,nkbin,kminc,kmaxc,nkbinc,[0,2,4],boxsize,boxsizes,frac_nyq, PS_type,WFCon_type)
    if(PS_type=='crs'):
        ...
        conv,kd,kc   =  ConvMat_Pypower( Xr,Yr,Zr, weitR,Ngrid,Pkcrs_norm,NrandRat,kmin,kmax,nkbin,kminc,kmaxc,nkbinc, [1,3],boxsize,boxsizes,frac_nyq, PS_type,WFCon_type)
    np.save(file_dir+'conv'+PS_type+'_'+WFCon_type+'_'+PS_measure,np.array([conv,kd,kc],dtype=object),allow_pickle=True)
