...
def LoadPS_Fun(Input_dir,Nmock,PS_type,WFCon_type,OnlyL0,PS_measure ): 
    ...
    w_rand = np.nan
    if( WFCon_type != 'Ross'):
        PS_type='den-024 mom-024 crsv-13'
        conv=[];Kdj=[];Kpj=[];Kcj=[]
        if('den' in PS_type ):
...
def ParmPSmod_Fun(parm,ps_type):
    if(ps_type=='den-024 mom-024')or(ps_type=='den-024 mom-02')or(ps_type=='den-02 mom-02')or(ps_type=='den-024 mom-024 crsv-13')or(ps_type=='den-02 mom-02 crsv-1'):
    ...
    if(ps_type=='den-02 mom-0')or(ps_type=='den-024 mom-0')or(ps_type=='den-0 mom-0 crsv-1'):
    ...
    if(ps_type=='den-0 mom-0'):
        ...
        sigmavv2_squre=sigmavv1_squre
        sigmavv3_squre=sigmavv1_squre     
    if(ps_type=='den-02 mom-0 crsv-1'):
...
def Plot_crsMulti(param_test ,Nk,fit_minK,fit_maxK,k_data,Pk_data,ePk,ekPk,Sig8_fid,PT,PS_type, az,Hz,Dz,kmod_conv,WF_Kobs_Kmod,WF_rand, WFCon_type ,Model_type,iplt ):    
     ...
     for i in range(len(Ps_type) ):
         ...
         ps_type.append(ps_type0[0]) 
         Nmt.append( len( ps_type0[1]))   
     Pkmodc , kmodc , Pkmod , kmod  =PkmodMulti_Fun(param_test,False,Sig8_fid, k_data, kmod_conv, WF_Kobs_Kmod,WF_rand,PT,'crsv-13', az,Hz,Dz, WFCon_type,Model_type )
     Nkm=len(kmod)  //2 
     k_data = k_data[:Nk]
     kmod   = kmod[:Nkm]
     kmodc  = kmodc[:Nk]
     ...
     indkobs= np.where((k_data >=fit_minK)&(k_data <=fit_maxK))[0]   
     if('crsv' in ps_type) :     
         intd=np.where(np.array(ps_type)=='crsv')[0]
         mut=np.array(mt)[intd][0]      
         for i in range(len(mut)):
             if('1' in mut):
                 mut=mut.replace('1','')
