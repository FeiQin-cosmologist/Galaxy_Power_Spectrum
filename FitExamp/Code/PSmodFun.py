...
def Pkmod_Fun(params,sigma8_fid,PT,ps_type, az,Hz,Dz, do_Multi=True, GaliTrans=False  ):  
    ...
    # model PS:
    P_model=[];PSTYP=[];PSRSD=[] 
    mu = np.linspace(-1.0, 1.0, Nmubin0) 
    if(not GaliTrans):
        if ('den' in ps_type):
            ...
            P_0den = (2.0*0 + 1.0)/2.  * sp.integrate.simps(P_den,mu,axis=1)
            P_2den = (2.0*2 + 1.0)/2.  * sp.integrate.simps( ((3.*(mu**2)-1.)/2.)*P_den,mu,axis=1)
            P_4den = (2.0*4 + 1.0)/2.  * sp.integrate.simps( ((35.*(mu**4)-30.*(mu**2)+3.)/8. )*P_den,mu,axis=1)  
        ...
        if ('mom' in ps_type):
            ...
            P_0mom = (2.0*0 + 1.0)/2.  * sp.integrate.simps(P_mom,mu,axis=1)
            P_2mom = (2.0*2 + 1.0)/2.  * sp.integrate.simps( ((3.*(mu**2)-1.)/2.)*P_mom,mu,axis=1) 
            P_4mom = (2.0*4 + 1.0)/2.  * sp.integrate.simps( ((35.*(mu**4)-30.*(mu**2)+3.)/8. )*P_mom,mu,axis=1)  
        ...
        if ('crs' in ps_type):
            ...
            P_1crs = (2.0*1 + 1.0)/2.  * sp.integrate.simps(mu*P_crs,mu,axis=1)
            P_3crs = (2.0*3 + 1.0)/2.  * sp.integrate.simps( (1./2.*(5.*mu**3-3.*mu) )*P_crs,mu,axis=1)   
